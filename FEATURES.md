...
- **`warden-identity::ca::issue` emits DNS SANs.** New `service_segment_of()` helper parses the last `/service/<name>` segment out of the SPIFFE URI and emits `DnsName(<name>)` + `DnsName("localhost")` alongside the existing `URI:` SAN. Mirrors the bootstrap cert SAN pattern from `gen_certs.sh` so workload SVIDs work as drop-in replacements for bootstrap certs on the receive side.
- **No spec change.** SPIFFE identity is still URI-based; the DNS SANs are a TLS hostname-verification accommodation, not a new identity surface.
- **Verified end-to-end.** Prod: `/version.json` returns `0.9.7`; landing page returns 200 (was 500); simulator → proxy → brain succeeds at 96.5% (was 0% with `Brain unreachable` on every fire); zero `SAN mismatch` / `NotValidForName` / `Brain unreachable` events across all 8 helper-using services. Dev: same posture.
- **Why the bug went undetected through v0.9.2 → v0.9.6.** Sessions 3a, 3b, 4, 4b, and 5 all "verified" via refresh logs (which hit identity's listener — bootstrap cert with DNS SANs — and worked). The actual workload-cert-as-server path was never exercised because simulator was paused by default and console's webauthn auth gated the dev handler.

### 14.27 v0.9.6 ship log (2026-05-14)

B8 session 5 — `SpiffeServerVerifier` flips from warn-mode to enforce. Closes [§Internal service mTLS §10 Q2]: every outbound caller now rejects a server cert whose SPIFFE SAN URI isn't on the operator-supplied allowlist, instead of letting the connection succeed with a `warn`-level log line. With session 4b shipping the last outbound surface (console + simulator via `HttpProvider`), every helper-using service now actually pays attention to the allowlist it carries.

- **Helper flips the single production callsite.** `build_refresh_client` in `warden-workload-identity/src/lib.rs:314` switches `CheckMode::Warn` → `CheckMode::Enforce`. Module docs + `CheckMode::Warn` doc-comment updated to reflect that `Warn` is now a test-only / staged-rollout posture, not the production default.
- **Empty-allowlist safety net retained.** Services without `WARDEN_<SVC>_EXPECTED_PEER_SPIFFE` set still fall through to chain-only verification (the `if self.expected.is_empty()` short-circuit in `SpiffeServerVerifier::verify_server_cert`). A partial rollout never silently breaks a service that hasn't been wired yet; the wired services enforce.
- **Pre-flight: console allowlist extended.** Until v0.9.6 console's allowlist read `service/identity` only — fine in warn-mode but post-flip would close every outbound to ledger / hil / sim / policy-engine. Extended dev + prod blocks to `identity,ledger,hil,policy-engine,simulator`. Proxy (4-way) and simulator (`identity,hil`) allowlists already covered their outbound surface from sessions 3b + 4b.
- **Verified end-to-end.** Dev: deployed v0.9.6, tailed every helper-using service's logs for `spiffe verifier: SAN mismatch` warnings — zero matches under normal load. Prod: same signal. The `/version.json` endpoint on `console-demo.vanteguardlabs.com` returns `0.9.6`.
- **Stolen-cert path now closes the connection.** Reproduction: presenting a `service/brain` cert at the `policy-engine` DNS name (whose SAN reads `service/policy-engine`) → reqwest returns `ApplicationVerificationFailure` and the connection dies. The proxy's outbound allowlist (`brain,policy-engine,hil,identity`) means brain *could* reach the same name with its own cert — the SAN-URI binds identity to URI, not DNS, so the closure is on the URI mismatch alone. Same defense applies symmetrically to console, simulator, and the inter-service-trusted services.

### 14.26 v0.9.5 ship log (2026-05-14)

B8 session 4b — outbound-side adoption for `warden-console` and `warden-simulator`. Closes the gap session 4 left open: the SDK clients used to cache a snapshot `reqwest::Client`, so the helper's per-refresh ArcSwap swaps never reached live callers. Now every backend client reads `provider.client()` per request.

- **`warden-sdk` introduces `HttpProvider`.** New `pub trait HttpProvider: Debug + Send + Sync { fn client(&self) -> Arc<reqwest::Client>; }` in `http` module. Every per-service client (`LedgerClient`, `AgentsClient`, `PoliciesClient`, `SimClient`, `WardenClient`) stores `Arc<dyn HttpProvider>` instead of `reqwest::Client`. `with_http_client(Client)` stays as a back-compat shim that wraps in `StaticHttpClient`; `with_http_provider(provider)` is the new entry point. `LedgerClient::http_client()` now returns `Arc<Client>` so the SSE streaming sites in console see the current credential. 49 lib + 16 integration tests pass; no external callers used `with_http_client` so the breaking change is internal-only.
- **`warden-console` adopts the helper.** When `WARDEN_CONSOLE_TLS_DIR` is set, boot constructs `Arc<WorkloadIdentity>` + spawns the refresh task, wraps it in a local `WorkloadHttp` adapter implementing `HttpProvider`, and threads one shared `Arc<dyn HttpProvider>` through every backend client (Ledger, HIL, Sim, Agents, Policies). One refresh task → all five clients pick up the rotated cert on the next outbound request. Legacy `WARDEN_CONSOLE_OUTBOUND_*_PATH` static-cert path stays available for non-mTLS dev deployments. Local `HilClient` (console-resident, not in SDK) mirrors the same provider shape.
- **`warden-simulator` adopts the helper.** New `http_provider` module exposes a `ClientFactory` closure type (`Arc<dyn Fn() -> reqwest::Client + Send + Sync>`); `SvidMinter` and the HIL auto-decide sidecar both store one and call `(self.http)()` per request. Boot picks `from_workload(wi)` or `from_static(client)` based on `WARDEN_SIM_TLS_DIR`. Per-agent proxy clients keep their own minted SVIDs (separate trust direction — agent identity, not service identity). 36 bin tests pass.
- **Both binaries install a rustls `CryptoProvider`.** The helper builds rustls config via `aws-lc-rs`; without `install_default()` at the top of `main`, rustls 0.23 panics on the first handshake. Both crates gain a direct `rustls = "0.23"` dep + the one-line preamble every other warden-* main already runs.
- **Compose env wired.** Both dev + prod blocks for console + simulator gain `WARDEN_<SVC>_TLS_DIR=/certs` + `WARDEN_<SVC>_WORKLOAD_REFRESH_URL=https://identity:8186/workload-svid` + `WARDEN_<SVC>_EXPECTED_PEER_SPIFFE`. `additional_contexts` extended with `warden-workload-identity` under console, simulator, *and* upstream-stub (the stub shares the simulator Dockerfile via two `[[bin]]` entries, so the BuildKit named-context COPY runs for both).
- **End-to-end verified.** Dev: both console and simulator emit `workload identity: refresh task spawned` followed by `workload SVID refresh: first refresh ok (bootstrap → workload)` with the right SPIFFE IDs (`spiffe://warden.local/service/console`, `spiffe://warden.local/service/simulator`). Prod: same signals, `/version.json` returns `0.9.5` on `console-demo.vanteguardlabs.com`. B8 outbound-mTLS hot-reload is now end-to-end across the active s2s callsites.

### 14.25 v0.9.4 ship log (2026-05-14)

B8 session 4 (partial) — receive-side adoption for `warden-ledger` and `warden-hil`. Both inbound mTLS listeners now use the helper's hot-reloading `WorkloadIdentity::rustls_server_config()`, mirroring the brain + policy-engine adoption in 3a.

- **`warden-ledger` adopts the helper.** `main.rs` swaps `build_server_config(dir)` for `WorkloadIdentity::from_env("ledger")` + `wi.start().await` + `wi.rustls_server_config()`. `src/tls.rs` deleted; `pub mod tls;` removed from `lib.rs`. Cargo.toml gains `warden-workload-identity` path-dep. Dockerfile gains the BuildKit named-context pattern.
- **`warden-hil` adopts the helper.** Same one-line callsite swap as ledger. `tls.rs` deleted; `pub mod tls;` removed. Cargo.toml + Dockerfile updated identically.
- **Compose env wired.** Both dev + prod blocks for ledger + hil gain `WARDEN_<SVC>_WORKLOAD_REFRESH_URL=https://identity:8186/workload-svid` + `WARDEN_<SVC>_EXPECTED_PEER_SPIFFE=spiffe://warden.local/service/identity`. Identity's `WARDEN_IDENTITY_ALLOWED_CALLERS` allowlist extended with `service/ledger` + `service/hil` so the two services can authenticate against the refresh endpoint.
- **cargo check + clippy + tests green.** No warnings; ledger 83/83 tests pass, hil 35/35 tests pass.
- **Session 4 scope reduced.** Console + simulator outbound adoption deferred to a follow-up session: their SDK clients (`LedgerClient`, `HilClient`, `AgentsClient`, `SimClient`) cache a snapshot `reqwest::Client` at boot via `with_http_client(client.clone())` — so swapping the underlying cert via `wi.reqwest_client()` would NOT propagate hot-reloads to live SDK callers. Proper adoption requires a `warden-sdk` refactor (closure / dynamic Client) which is outside session 4's bounded scope. The two services keep their existing static `OutboundTls::from_env()` path for now; cert rotation requires container restart, same posture as today. Deep-review + demo-mint are NATS-only (no peer mTLS hops), nothing to adopt.
...
- **Helper gains `rustls_server_config()`.** New `warden-workload-identity::server_config` module: `ArcSwapResolver` implements `rustls::server::ResolvesServerCert` and reads `Arc<ArcSwap<CertPair>>` on every TLS handshake. The returned `Arc<rustls::ServerConfig>` is built once at boot; refresh hits the next handshake without listener restart and never disrupts in-flight requests. Client-cert verifier is rooted at the bootstrap CA (which signs both bootstrap and workload SVIDs) so the existing SPIFFE-allowlist middleware works through the swap.
- **Helper gains `from_env_explicit(env_prefix, cert_slug)`.** Policy-engine reads `WARDEN_POLICY_*` env vars but ships `service-policy-engine.{crt,key}` cert files — the asymmetry is original to the repo. The explicit constructor decouples the env-var prefix from the cert filename slug; `from_env(svc)` is the symmetric default that delegates here with both equal.
- **Brain receive-side adoption.** `warden-brain/src/main.rs` replaces `build_server_config(cert_dir)` with `WorkloadIdentity::from_env("brain")` + `wi.start().await` + `wi.rustls_server_config()`. The crate's now-dead `tls.rs` module + its `pub mod tls;` line are deleted. Cargo.toml gains `warden-workload-identity = { path = "../warden-workload-identity" }`. Dockerfile gains the BuildKit named-context pattern matching warden-identity.
- **Policy-engine receive-side adoption.** Same one-line callsite swap as brain, via the explicit `from_env_explicit("POLICY", "policy-engine")` constructor. `tls.rs` deleted; Dockerfile + Cargo.toml updated identically.
- **Compose env wired.** Both dev + prod compose blocks for brain and policy-engine gain `WARDEN_<SVC>_WORKLOAD_REFRESH_URL=https://identity:8186/workload-svid` + `WARDEN_<SVC>_EXPECTED_PEER_SPIFFE=spiffe://warden.local/service/identity`. Identity's `WARDEN_IDENTITY_ALLOWED_CALLERS` allowlist gains `service/brain` and `service/policy-engine` so the two services can authenticate against the refresh endpoint.
- **Session 3b ahead.** Proxy outbound side (`reqwest::Client` for brain/policy/hil/identity calls) still uses the legacy `OutboundTls` path-bundle. The helper's `reqwest_client()` is ready; the proxy adoption involves replacing the `OutboundTls` plumbing across `LayerClient` / `SigningClient` / `A2aClient` / the HIL poll path. That refactor lives in its own patch to keep this diff bounded and verifiable in isolation.
...
- **`warden-workload-identity` crate exists.** New workspace sibling under `repos/warden-workload-identity`. Public surface per spec §5: `WorkloadIdentity::from_env(service)`, `start()`, `reqwest_client()`, `current_spiffe()` / `current_kind()` / `current_not_after()`, `shutdown()`. Internals: `CertPair` struct (PEM + DER views + SPIFFE URI + lifetimes), `ArcSwap<Arc<CertPair>>` for lock-free atomic swap, background refresh task with 50% remaining-TTL window (§11 Q3 lock) and `1s/4s/16s` retry backoff, custom `rustls::client::danger::ServerCertVerifier` wrapping the standard `WebPkiServerVerifier` with a post-step SAN-URI allowlist check (warn-mode in sessions 2–4 per §11 Q5; flips to enforce in session 5). 9 unit tests; clippy `-D warnings` clean.
- **§11 Decided 2026-05-14 inlined at the bottom of TECH_SPEC §Workload SVID refresh.** All six open questions locked: Q1 helper location = new crate; Q2 default TTL = 1800 s; Q3 refresh window = 50 % remaining TTL; Q4 sampling = every refresh; Q5 client-side check = warn-mode sessions 2–4 → enforce session 5; Q6 NATS-client SVID = reuse workload SVID for handshake.
- **`POST /workload-svid` lands on `warden-identity` (mTLS-authed, internal router).** New `src/workload_svid.rs` modeled on `src/svid.rs` but without attestation evidence — the mTLS handshake's verified SAN URI is the auth signal, and the issued cert's SAN is copied verbatim from the caller's. Body `{ ttl_seconds }` optional (defaults to 1800 s, capped at 3600 s). Caller allowlist via new `WARDEN_IDENTITY_WORKLOAD_ALLOWED_CALLERS` env, defaulting to `WARDEN_IDENTITY_SIGN_ALLOWED_CALLERS` when unset (spec §3). `caller_kind` derived server-side from the presented cert's validity-period (≤24 h → workload, else bootstrap) so the operator-relevant "stuck on bootstrap" signal is honest. New `workload_svids` SQLite table with three indexes (spiffe_id, not_after, caller_kind).
- **Chain v3 `svid.workload_refreshed` event** emitted on every successful `/workload-svid` mint, routed through the existing `agents_ledger::prepare_lifecycle_emission` outbox so the durability story is identical to the WAO lifecycle events (commit in same SQLite tx as the workload row; NATS publish best-effort; sweeper drains on outage recovery). Payload carries `issued_id`, `spiffe_id`, `caller_kind`, `ttl_seconds`, `issued_at`, `expires_at`. Console's `summarize_lifecycle_payload` gains a one-liner so audit-page readers see `workload SVID refreshed (from bootstrap, ttl 1800s)` instead of `—`.
- **Identity adopts the helper as the first caller.** `warden-identity/src/main.rs` constructs a `WorkloadIdentity::from_env("identity")` when `WARDEN_IDENTITY_TLS_DIR` is set, calls `start()` to spawn the refresh task. When `WARDEN_IDENTITY_WORKLOAD_REFRESH_URL` is unset the helper loads bootstrap but `start()` returns `RefreshDisabled` — service runs identical to v1.x+2 (operator opt-in by setting one env var). Listener allowlist (`WARDEN_IDENTITY_ALLOWED_CALLERS`) gains `spiffe://warden.local/service/identity` so the receive path accepts identity's own bootstrap + workload certs. `mtls::CallerIdentity` extension gains `cert_validity_secs` so handlers can dispatch on cert kind without re-parsing the leaf per request.
- **Compose env wired.** Both `warden-e2e/dev` and `warden-e2e/prod` `identity` service gain `WARDEN_IDENTITY_WORKLOAD_REFRESH_URL=https://identity:8186/workload-svid` and `WARDEN_IDENTITY_EXPECTED_PEER_SPIFFE=spiffe://warden.local/service/identity`. Build context grows `additional_contexts: warden-workload-identity=../../warden-workload-identity` mirroring the proxy/sandbox pattern. Dockerfile lays both repos as siblings under `/build/` per the same BuildKit named-context convention.
- **Session 3+4 unblocked.** Helper crate exists, `/workload-svid` works, identity proves the loop. Session 3 rolls the helper through proxy/brain/policy-engine (one-line callsite swap from `RustlsConfig::from_pem` to helper-owned). Session 4 covers ledger/hil/console/simulator/deep-review/demo-mint. Session 5 flips `EXPECTED_PEER_SPIFFE` to enforce across the stack.

Why a minor bump (0.8.5 → 0.9.0)? Per CLAUDE.md convention: minor = a new roadmap-checklist feature axis lands in the codebase. The crate is new, the endpoint is new, the chain-v3 event is new, and identity's process now carries a refresh background task that did not exist in v0.8.5. Patch would understate the change footprint.

### 14.20 v0.8.5 ship log (2026-05-14)

B8 session 1 — design lock for the v1.x+3 workload-SVID refresh slice. Spec only, no code in this commit. Mirrors the B7 session-1 pattern (v0.7.1): pre-agree the wire shape so sessions 2-5 have a fixed target.

- **TECH_SPEC.md** gains a new top-level `## Workload SVID refresh` section between `## Internal service mTLS` and `## Threat model`. Contents: §1 what this closes (G1 leaked-cert revocation window, G2 server-identity validation gap, G3 Vault rotation propagation); §2 hops in scope (every service holding a bootstrap cert plus the `nats` server's deliberate exclusion); §3 wire shape (`POST /workload-svid` on warden-identity, mTLS-authed at the listener so the cert's SAN URI is what gets re-issued — caller cannot ask for another service's identity); §4 caller-side refresh state machine (boot on bootstrap → first refresh → background sweep at 50% remaining TTL → 3-retry backoff → fallback to bootstrap if SVID expires with no successful refresh); §5 SDK helper crate placement (Option A `warden-sdk::mtls` vs Option B new `warden-workload-identity` — Option B is the recommended default, locked in session 2); §6 server-side SPIFFE SAN-URI check on the client side (closes B7-§10 Q2 — strict-equality validation of peer cert's SAN URI against `WARDEN_<SVC>_EXPECTED_PEER_SPIFFE`, defaults to warn-mode so v1.x+2 posture is preserved if the env is unset); §7 chain v3 `svid.workload_refreshed` event (`caller_kind=bootstrap|workload` distinguishes "first refresh after boot" from "background refresh", which makes "service is stuck on bootstrap" queryable); §8 failure & fallback matrix; §9 5-session implementation roadmap; §10 explicit non-goals (CA-root hot-reload, NATS-server SVID issuance, per-tenant allowlists, workload-SVID-backed `/sign`, revocation feed); §11 six open questions to lock in session 2.
- **Contents** TOC gains the new section entry between Internal service mTLS and Threat model. Internal-service-mTLS entry's parenthetical updates from "designed; implementation deferred to v1.x+2" to "shipped v0.8.3 — application hops; v0.8.4 — NATS transport".
- **No FEATURES.md cross-references update** in this session — the v1.x+1 audit catalog (§14.4) and the threat-model open-items (§14.5) already pointed at v1.x+3 as the home for SVID refresh; the new spec section is now the resolution target those entries reference.
- **No code changes.** No service repos touched. Sessions 2-5 land the helper crate, the new endpoint, per-service rollout, and the client-side SAN-URI enforce flip.

Why a patch bump (0.8.4 → 0.8.5) rather than a minor (0.9.0)? Per the CLAUDE.md convention, patch = bugfix / polish / discoverability work on top of shipped behaviour, minor = a new roadmap-checklist feature. This session is discoverability for the v1.x+3 work — visitors of `/version.json` see the spec is moving but no shipped surface changed. The minor bump will come with session 2 when the helper crate actually exists in the workspace.

### 14.19 v0.8.4 ship log (2026-05-14)

B7.5 — NATS TLS+mTLS shipped. Closes the last plain-text leg in the warden stack: every forensic publish, JetStream consumer, and `demo.session_minted` event now rides an authenticated TLS channel. Together with B7 sessions 1-6 this completes the "every internal leg is mTLS-gated" goal from spec §"Internal service mTLS".

- **NATS server binds TLS+mTLS on `:4222`.** New `warden-e2e/{prod,dev}/nats-server.conf` config files: `cert_file: /certs/service-nats.crt`, `key_file: /certs/service-nats.key`, `ca_file: /certs/ca.crt`, `verify: true` (rejects unauthenticated clients), 5s handshake timeout. Compose `nats` service bind-mounts the cert bundle read-only and the config; `command: ["-c", "/etc/nats/nats-server.conf"]` overrides the default `nats:2` entrypoint.
- **`gen_certs.sh`** SERVICES list grows `nats` and `demo-mint`: every NATS-connecting workload (including the server itself) now has a SPIFFE-tagged cert pair.
- **`warden-proxy` + 6 sibling repos gain `src/nats_tls.rs`.** Identical helper across `warden-proxy`, `warden-ledger`, `warden-hil`, `warden-identity`, `warden-policy-engine`, `warden-deep-review`, `warden-demo-mint`. Reads `NATS_TLS_{CERT,KEY,CA}_PATH`; when all three are set, returns an `async_nats::ConnectOptions` with `require_tls(true) + add_client_certificate(cert, key) + add_root_certificates(ca)`. Partial config errors at boot. When unset, returns the default options (plain TCP) — preserves the legacy host-cargo dev path.
- **Connect callsites updated.** Each repo's `async_nats::connect(...)` swapped for `async_nats::connect_with_options(url, build_nats_options()?)`. Best-effort paths in HIL / identity / demo-mint keep their original `Ok/Err` arms; the helper is built once before the match.
- **Compose flips.** Every NATS-connecting service in both `prod` and `dev` stacks gets the three new env vars (`NATS_TLS_CERT_PATH=/certs/service-X.crt` + KEY + CA). Deep-review and demo-mint additionally bind-mount the certs directory (they didn't need /certs before this session). Dev uses `certs-dev/` so a prod NATS client cert cannot publish to dev's NATS server and vice versa.
- **Helm chart 0.7.1** (`appVersion: 0.8.4`). `warden.backendEnvs` grows a trailing block that, when `tlsBundle.secretName` is set, emits the three NATS_TLS env vars for each of the six in-chart NATS-connecting services (proxy, ledger, hil, identity, policy-engine, deep-review). Cert path is computed from the kebab-cased service name (`service-policy-engine.crt`, `service-deep-review.crt`).
- **Spec lock.** TECH_SPEC.md §"Internal service mTLS" §8 "Out of scope" — the NATS bullet flips from "Tracked as B7.5" to "Shipped v0.8.4" with the implementation summary inline. Module-status header trailing sentence updated: every internal leg is now mTLS-authenticated.
- **Verification.** Dev + prod deploys come up clean. NATS boot log reports TLS handshake; every service logs "NATS mTLS enabled cert=/certs/service-X.crt …" on connect. Simulator stats stabilize at the usual ~90% success rate. Forensic events continue to flow into the ledger (no chain breakage). `nats:2` server logs `TLS handshake error` for any unauthenticated test client.

### 14.18 v0.8.3 ship log (2026-05-14)

B7 session 6 — final session of the internal-s2s-mTLS arc. Every internal application hop is now mTLS-gated end-to-end. HIL + identity flip from CODE-only to live deployment; simulator + proxy gain outbound TLS for the last two backends (HIL, identity) so the session-5 receive code can be turned on without breaking the demo.

- **`gen_certs.sh` mints `service-simulator.{crt,key}`.** Single one-line change — appended `simulator` to the `SERVICES` list. CN=`warden-simulator`, SAN `URI:spiffe://warden.local/service/simulator`. Same 1-year lifetime and 644-perm keys as every other workload bootstrap cert. Re-run against both `certs/` (prod) and `certs-dev/` to populate the bind-mounts.
- **`warden-simulator` outbound mTLS — new `outbound_tls.rs`.** Mirrors `warden-console/src/outbound_tls.rs`: reads `WARDEN_SIM_OUTBOUND_{CERT,KEY,CA}_PATH`; partial-set is a boot error; builds one shared `reqwest::Client` with `Identity::from_pem(cert+key)` + `add_root_certificate(ca)`. Threaded into the three s2s callsites:
  - `SvidMinter::new(...)` — was building its own client; now takes the shared client as a parameter (called for every per-cycle SVID mint).
  - `enroll::enroll_personas(...)` — was building its own client; now takes the shared client as a parameter (called once at boot per persona instance).
  - `boot::try_spawn_hil_sidecar(...)` — was building its own client; now takes the shared client as a parameter (driving the `/pending` poll + `/decide/{id}` posts that keep the Yellow-tier queue draining).
  - Vault + per-agent proxy hops keep their own clients — Vault is plain HTTP, and per-agent proxy mTLS uses the agent SVID, a separate trust direction.
- **`warden-proxy` outbound mTLS extended to identity + HIL.** `fork::OutboundTls` gets `#[derive(Clone)]` so it can be threaded into multiple call sites. New `fork::build_outbound_client(timeout: Option<Duration>, tls: Option<&OutboundTls>)` helper extracts the PEM-loading dance from `LayerClient::new`; `SigningClient::new`, `A2aClient::new`, and the HIL poll path (`state.http`) all use it. Result: a single `service-proxy` workload cert authenticates the proxy against brain, policy-engine, hil, identity, and ledger — five backend hops, one bundle. `timeout = None` preserves the original "no per-request timeout" posture on the HIL poll loop (the outer cap is `state.hil_poll_timeout`).
- **`warden-hil` deploys mTLS receive.** Compose flips `WARDEN_HIL_TLS_DIR=/certs` + `WARDEN_HIL_ALLOWED_CALLERS=spiffe://warden.local/service/{proxy,console,simulator}` + `WARDEN_HIL_HEALTH_ADDR=0.0.0.0:9084` on both `prod` and `dev` stacks. Port `:8084` becomes rustls; health + `/readyz` + `/metrics` move to `:9084` (kubelet + Prometheus reach the plain port without a client cert). HIL bind-mounts `../../warden-proxy/certs:/certs:ro` so `service-hil.{crt,key}` are readable. Code-side changes shipped in session 5 — no library changes needed.
- **`warden-identity` deploys mTLS receive — dual-listener.** Same pattern as ledger: plain `:8086` strips the internal `/svid` + `/grant` + `/revoke` + `/sign` + `/actor-token*` + `/agents*` routes; mTLS `:8186` carries the full surface gated by the SPIFFE allowlist. Compose flips `WARDEN_IDENTITY_TLS_DIR=/certs` + `WARDEN_IDENTITY_ALLOWED_CALLERS=...` + `WARDEN_IDENTITY_MTLS_ADDR=0.0.0.0:8186`. Public routes (`/jwks.json`, `/.well-known/spiffe-bundle`, `/stats`, health) stay reachable on `:8086`.
- **URL scheme flips — every caller.** Compose now has:
  - Proxy: `WARDEN_HIL_URL=https://hil:8084`, `WARDEN_IDENTITY_URL=https://identity:8186`.
  - Console: `WARDEN_CONSOLE_HIL_URL=https://hil:8084`, `WARDEN_CONSOLE_IDENTITY_URL=https://identity:8186`.
  - Simulator: `WARDEN_HIL_URL=https://hil:8084`, `WARDEN_SIM_SVID_URL=https://identity:8186`, `WARDEN_SIM_ENROLL_IDENTITY_URL=https://identity:8186`.
  - Console no longer documents an "HIL plain HTTP because simulator…" caveat — the simulator's caveat is gone.
- **Helm chart `0.7.0`** (`appVersion: 0.8.3`). `warden.backendEnvs` extends:
  - **HIL arm** added: `WARDEN_HIL_TLS_DIR`, `WARDEN_HIL_ALLOWED_CALLERS` (`proxy,console,simulator`), `WARDEN_HIL_HEALTH_ADDR=0.0.0.0:9084` when `tlsBundle.secretName` is set.
  - **Identity arm** added: `WARDEN_IDENTITY_TLS_DIR`, `WARDEN_IDENTITY_ALLOWED_CALLERS` (same triple), `WARDEN_IDENTITY_MTLS_ADDR=0.0.0.0:8186`.
  - **Proxy arm**: `WARDEN_HIL_URL` and `WARDEN_IDENTITY_URL` schemes/ports flip with the bundle (`https://...:8186` for identity, https for hil).
  - **Console arm**: `WARDEN_CONSOLE_HIL_URL` + `WARDEN_CONSOLE_IDENTITY_URL` flip with the bundle.
  - **`values.yaml`** gains `services.hil.healthPort: 9084` and `services.identity.mtlsPort: 8186`, both documented inline. Service template still exposes a single port — the dual-port Service work stays its own chart minor.
- **Verification.** Both prod and dev deploys come up clean. HIL boot logs report `warden-hil mTLS listening on 0.0.0.0:8084 (allowlist=…)` with the plain health port alongside. Identity boots reporting both listeners. Simulator + proxy + console boot logs confirm their respective `outbound mTLS enabled` lines. 3-way SPIFFE curl tests on the HIL + identity mTLS ports (no-cert TLS-fail, wrong-SAN 401 `uri_not_allowlisted`, correct-SAN through to the 4xx/2xx body parse) pass on both stacks. Simulator stats stabilize at the usual 88-90% success rate, p50 ~10ms; HIL `/pending` queue drains.

### 14.17 v0.8.2 ship log (2026-05-14)

B7 session 5 — ledger receive + console outbound. The console→ledger hop and proxy→ledger HTTP hop (the audit-write surface) are now mTLS-gated with SPIFFE-URI caller identity. Console gains a typed outbound TLS helper shared across every backend hop. HIL + identity receive code is shipped but deployment is held for session 6 — see "Deferred to session 6" below.

- **`warden-ledger` mTLS receive — dual-listener pattern.** New module `mtls::SpiffeAcceptor` (wraps `RustlsAcceptor` from `axum-server`) + `mtls::CallerAllowlist` (reads `WARDEN_LEDGER_ALLOWED_CALLERS`) + `require_allowed_caller` middleware. `WARDEN_LEDGER_TLS_DIR=/certs` flips the binary to **two listeners**:
  - **Plain HTTP on `:8083`** (UNCHANGED port) serves only `/verify`, `/audit/{agent_id}`, `/audit/{agent_id}/count`, `/audit/agent/.../lifecycle`, `/health`, `/readyz`, `/metrics`. The internal write + console-only read subset is **stripped** from this router so an attacker on the docker network gets a 404 if they hit `/log` on `:8083` — they cannot bypass mTLS by hitting the plain port. Caddy + kubelet + Prometheus reach this listener unchanged.
  - **mTLS on `:8183`** (new, `WARDEN_LEDGER_MTLS_ADDR`) serves the **full** router. Per-route SPIFFE-allowlist middleware fires on `/log`, `/agents`, `/audit/correlation/{id}`, `/stream/audit`, `/export`, `/exports`, `/export/regulatory` — every internal-only path. The public read surface is also reachable here (so console can use one URL); SPIFFE check doesn't run on those. Allowlist: `service/proxy`, `service/console`, `service/deep-review`.
  - Empty allowlist with TLS on → boot panic. 4 new unit tests covering allowlist parse + permit semantics; 83 ledger lib tests pass; clippy clean.
  - `build_app_split` returns `(internal, public, health)`; existing `build_app` merges all three for backward compat (tests + plain-HTTP boot).
- **`warden-console` outbound mTLS.** New `outbound_tls` module mirrors `warden-proxy/src/fork.rs::OutboundTls`. Reads `WARDEN_CONSOLE_OUTBOUND_{CERT,KEY,CA}_PATH` at boot; partial-set is a fail-fast error (a half-rolled cert deploy should not silently bypass mTLS to one backend). Builds **one** `reqwest::Client` with `Identity::from_pem(cert+key concat)` + `add_root_certificate(ca)`; injected into `LedgerClient`, `HilClient`, `AgentsClient`, `PoliciesClient`, `SimClient` via their existing `with_http_client` methods. One TLS handshake amortizes across every console→backend hop (reqwest's connection pool keys per-Client).
- **`WARDEN_CONSOLE_POLICY_ENGINE_URL` scheme flip — latent session-4 bug fix.** Policy-engine has bound TLS on `:8082` since v0.8.1, but the compose + helm `WARDEN_CONSOLE_POLICY_ENGINE_URL` was still `http://policy-engine:8082`, which silently broke `/policies` reads in the console (TLS port returns TLS errors on plain HTTP). Flipped to `https://policy-engine:8082`; the new outbound mTLS client makes this work. Policy-engine's `ALLOWED_CALLERS` also extended to include `service/console` for the `/policies/*` management surface.
- **`warden-hil` + `warden-identity` mTLS receive — CODE shipped, deployment deferred.** Both repos now carry `mtls.rs` + `tls.rs` + `build_app_split` + a dual-listener `main.rs`. HIL runs single-mode mTLS (no browser routes go through Caddy — all callers are internal); identity runs the same dual-listener pattern as ledger (`/jwks.json` + `/.well-known/spiffe-bundle` + `/stats` stay public on the plain port). Env-gated on `WARDEN_HIL_TLS_DIR` / `WARDEN_IDENTITY_TLS_DIR`; both env vars are **left unset in compose** for now. Reason: the simulator's auto-decider sidecar polls HIL `/pending` and POSTs `/decide` over plain HTTP, and the per-agent SVID flow hits `identity/svid`. Updating simulator's outbound TLS (mint `service-simulator.{crt,key}` + add an outbound-TLS helper to `boot.rs`) is the session-6 deliverable; flipping the env vars then is a one-liner.
- **Helm chart 0.6.0** (`appVersion: 0.8.2`). `warden.backendEnvs` extends:
  - **Ledger:** injects `WARDEN_LEDGER_TLS_DIR` + `WARDEN_LEDGER_ALLOWED_CALLERS` (`proxy,console,deep-review`) + `WARDEN_LEDGER_MTLS_ADDR=0.0.0.0:8183` when `tlsBundle.secretName` is set.
  - **Console:** `WARDEN_CONSOLE_LEDGER_URL` scheme + port flip when TLS on (→ `https://...:8183`); `WARDEN_CONSOLE_POLICY_ENGINE_URL` scheme flip (→ `https://...:8082`); `WARDEN_CONSOLE_OUTBOUND_{CERT,KEY,CA}_PATH` populated from `tlsBundle.mountPath`.
  - **Policy-engine:** `WARDEN_POLICY_ALLOWED_CALLERS` extended to include `service/console`.
  - **`values.yaml`:** `services.ledger.mtlsPort: 8183` added with a comment noting the Service template still exposes only `port` — operators wanting console→ledger over mTLS on this chart need to add a second Service port. A later chart minor will land the dual-port Service block.
- **Compose flipped** in both `warden-e2e/{prod,dev}`. Ledger mounts certs read-only and gets the three TLS env vars; exposes `8183` on the host for 3-way SPIFFE curl tests. Console mounts certs read-only and flips its ledger + policy URLs (`WARDEN_CONSOLE_LEDGER_URL=https://ledger:8183`, `WARDEN_CONSOLE_POLICY_ENGINE_URL=https://policy-engine:8082`). HIL + identity blocks document the deferred TLS_DIR env with a load-bearing comment so a future operator doesn't enable them without minting simulator's cert first.
- **Cargo deps** — ledger, hil, identity each gain `rustls = { version = "0.23", default-features = false, features = ["aws-lc-rs"] }`, `tokio-rustls`, `rustls-pemfile`, `axum-server`, `x509-parser`. Same versions as brain + policy-engine so the future shared `warden-mtls-receive` crate refactor is a clean lift.
- **Lib-test sweep clean** across all four repos at session-5 head:
  - `warden-hil`: 35 lib tests pass (4 new in `mtls::tests`).
  - `warden-ledger`: 83 lib tests pass (4 new in `mtls::tests`).
  - `warden-identity`: 197 lib tests pass (4 new in `mtls::tests`).
  - `warden-console`: builds + clippy clean.

**Deferred to session 6** (one chunk, with a clean unblock path):
1. Mint `service-simulator.{crt,key}` (gen_certs.sh already iterates a `SERVICES` list — append one name).
2. Add an `outbound_tls` module to `warden-simulator/src/boot.rs` mirroring `warden-console/src/outbound_tls.rs`; the HIL HTTP client + the SVID-mint client + the enrollment client all consume it.
3. Flip simulator's `WARDEN_HIL_URL`, `WARDEN_SIM_SVID_URL`, `WARDEN_SIM_ENROLL_IDENTITY_URL` schemes; set the three OUTBOUND envs.
4. Add `WARDEN_HIL_TLS_DIR=/certs` + `WARDEN_HIL_ALLOWED_CALLERS=spiffe://warden.local/service/proxy,spiffe://warden.local/service/console,spiffe://warden.local/service/simulator` to compose. Same for identity (`/agents` + `/svid` + `/grant`).
5. E2E verify + cert-pair rotation runbook + spec lock — the original session-6 scope.

### 14.16 v0.8.1 ship log (2026-05-14)

B7 session 4 — policy-engine receive. The proxy→policy hop now matches the proxy→brain hop's posture: rustls + SPIFFE-URI caller identity on the application port, plain-HTTP `/health` + `/readyz` + `/metrics` on a separate port. Second backend on the substrate; the pattern is converging.

- **`warden-policy-engine` mTLS receive.** `WARDEN_POLICY_TLS_DIR=/certs` flips the engine from plain HTTP to `axum-server` + `rustls` on `:8082`. Loads `ca.crt` + `service-policy-engine.{crt,key}`; `WebPkiClientVerifier` validates the chain. New `auth::SpiffeAcceptor` + `auth::require_allowed_caller` middleware mirror brain's session-3 implementation; the only material differences are the env-var names (`WARDEN_POLICY_*`) and the route surface guarded (`/evaluate` plus the entire `/policies/*` management surface — both behind the allowlist). Empty allowlist with TLS on → boot panic. 4 new unit tests covering allowlist parse + permit semantics; clippy clean.
- **`create_app_split`** added alongside `create_app`. Mirror of brain's split: `(app_router, health_router)` so main.rs can bind the application surface and `/` + `/health` + `/readyz` to separate ports. Existing `create_app` preserved for plain-HTTP mode and integration tests; it now merges the two routers internally rather than re-defining routes.
- **`warden-proxy` outbound.** *No code change.* The session-3 `OutboundTls` wiring (reqwest with `use_rustls_tls()` + `Identity::from_pem(cert+key)` + `add_root_certificate(ca)`) already covers every backend reqwest URL — the proxy presents `service-proxy.{crt,key}` to brain *and* policy with one client. Flipping `WARDEN_POLICY_URL` from `http://` to `https://` is sufficient; the cert path triplet is unchanged.
- **Helm chart 0.5.0** (`appVersion: 0.8.1`). `warden.backendEnvs` auto-injection extended:
  - Proxy: `WARDEN_POLICY_URL` scheme follows `tlsBundle.secretName` (now `https://` when set, mirroring `WARDEN_BRAIN_URL`).
  - Policy-engine: `WARDEN_POLICY_TLS_DIR` + `WARDEN_POLICY_ALLOWED_CALLERS=spiffe://warden.local/service/proxy` + `WARDEN_POLICY_HEALTH_ADDR=0.0.0.0:9082` injected when `tlsBundle.secretName` is set.
  - `services.policyEngine.healthPort: 9082` added to `values.yaml`.
  - **Latent helper bug fixed.** `warden.probe` + `warden.metricsAnnotations` previously defaulted to `services.<name>.port` and ignored `healthPort`, which meant brain probes under TLS would target the mTLS port `:8081` (kubelet has no client cert). Both helpers now fall back through `probes.port → healthPort → port` (and `metrics.port → healthPort → port`). Brain probes now land on `:9081`; policy-engine on `:9082`. Proxy is unchanged (`probes.port: 8080` was already explicit). Caught while wiring policy-engine; folded into the chart bump.
- **Compose flipped** in both `warden-e2e/{prod,dev}` — policy-engine mounts certs read-only and gets the three TLS env vars; proxy gets `WARDEN_POLICY_URL=https://`. The proxy's outbound cert path triplet was already in place from session 3 and needed no edit.
- **Cargo deps** — policy-engine gains `rustls = { version = "0.23", default-features = false, features = ["aws-lc-rs"] }`, `tokio-rustls`, `rustls-pemfile`, `axum-server`, `x509-parser`. Same versions as brain so a future shared crate is a clean refactor.
- **Hil + ledger receive deferred to session 5.** Both ship browser-facing routes on the same listener (WebAuthn approver UI + demo-session-scoped `/audit` reads) that need a dual-mode listener (`WebPkiClientVerifier::allow_unauthenticated()` so the rustls handshake accepts both certified service callers and bare browsers; per-route middleware then SPIFFE-gates `/pending`/`/decide`/`/log` while leaving browser routes to existing auth). Pairing that work with the console-outbound rollout in session 5 keeps the dual-mode question in one place. **Deep-review needs no receive-side mTLS** — its only HTTP surface is `/metrics` + `/health` on a Prometheus-scrape port, and every inter-service touch happens via NATS pubsub.

Sessions 5-6 queued: hil + ledger receive with dual-mode listener, console outbound + identity SVID validation (session 5); e2e verify + cert-pair rotation runbook + spec lock (session 6).
...
- **`warden-proxy/scripts/gen_certs.sh`** now mints 8 per-service workload cert pairs (`service-{proxy,brain,policy-engine,hil,identity,ledger,deep-review,console}.{crt,key}`) under the same warden CA. Each carries its SPIFFE identity in the SAN URI extension (`URI:spiffe://warden.local/service/<name>, DNS:<name>, DNS:localhost`). RSA-2048, 1-year lifetime per Q1 decision. `-copy_extensions copy` on the issuing `openssl x509` preserves the SAN through CSR → cert (without it the issued cert has no SAN and SPIFFE-aware verifiers reject as non-workload). Existing `server.{crt,key}` (proxy agent-facing) and `client.{crt,key}` (legacy starter-agent) unchanged. `set -euo pipefail`; CSRs auto-cleaned after issuance.
- **`warden-charts/charts/warden/`** renames `values.proxyTls` → `values.tlsBundle` (chart 0.2.0 → 0.3.0; appVersion 0.7.0 → 0.7.3). Templates use k8s Secret `items:` projection so each backend pod mounts only `ca.crt` + its own `service-<name>.{crt,key}`. The proxy additionally projects `server.{crt,key}` + `client.{crt,key}`. A compromised brain pod cannot read hil's private key. `helm lint` clean; default render + with-tls render produce 20 kinds each.
- **`warden-e2e/{prod,dev}/docker-compose.yml`** extends `WARDEN_IDENTITY_SIGN_ALLOWED_CALLERS` from `spiffe://warden.local/proxy/` to `spiffe://warden.local/proxy/,spiffe://warden.local/service/` so the future bootstrap-cert callers will pass the prefix-match check on `/sign`. Identity service code unchanged — its parser was already comma-aware.
- **Spec hygiene** — `TECH_SPEC.md` §7 (Helm chart wiring) rewritten with the actual file-set; §9 (Implementation roadmap) gains a Status column with session 1 + 2 marked Shipped, sessions 3-6 Queued. §"Module status" header notes session 2 done.
